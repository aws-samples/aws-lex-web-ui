...
var dist_es = __webpack_require__(24580);
...
function fromCognitoIdentity(parameters) {
    return async () => {
        ...
        const { GetCredentialsForIdentityCommand, CognitoIdentityClient } = await Promise.resolve(/* import() */).then(__webpack_require__.bind(__webpack_require__, 8738));
...
function throwOnMissingAccessKeyId() {
    throw new dist_es/* CredentialsProviderError */.C1("Response from Amazon Cognito contained no access key ID");
}
function throwOnMissingCredentials() {
    throw new dist_es/* CredentialsProviderError */.C1("Response from Amazon Cognito contained no credentials");
}
function throwOnMissingSecretKey() {
    throw new dist_es/* CredentialsProviderError */.C1("Response from Amazon Cognito contained no secret key");
}
...
function localStorage_localStorage() {
...
function fromCognitoIdentityPool({ accountId, cache = localStorage_localStorage(), client, clientConfig, customRoleArn, identityPoolId, logins, userIdentifier = !logins || Object.keys(logins).length === 0 ? "ANONYMOUS" : undefined, logger, parentClientConfig, }) {
    logger?.debug("@aws-sdk/credential-provider-cognito-identity", "fromCognitoIdentity");
    const cacheKey = userIdentifier
        ? `aws:cognito-identity-credentials:${identityPoolId}:${userIdentifier}`
        : undefined;
    let provider = async () => {
        const { GetIdCommand, CognitoIdentityClient } = await Promise.resolve(/* import() */).then(__webpack_require__.bind(__webpack_require__, 8738));
...
function throwOnMissingId() {
    throw new dist_es/* CredentialsProviderError */.C1("Response from Amazon Cognito contained no identity ID");
}

;// CONCATENATED MODULE: ../../../../../../node_modules/@aws-sdk/credential-providers/dist-es/fromCognitoIdentityPool.js

const fromCognitoIdentityPool_fromCognitoIdentityPool = (options) => fromCognitoIdentityPool({
    ...options,
});
...
class IframeComponentLoader {
  ...
  async getCredentials(poolId, region, logins) {
    if (logins) {
      ...
      const credentialProvider = fromCognitoIdentityPool_fromCognitoIdentityPool({
        identityPoolId: poolId,
        logins: logins,
        clientConfig: {
          region: region
...
const {
  CognitoIdentityClient: fullpage_component_loader_CognitoIdentityClient,
  GetIdCommand: fullpage_component_loader_GetIdCommand,
  GetCredentialsForIdentityCommand: fullpage_component_loader_GetCredentialsForIdentityCommand
} = __webpack_require__(83615);
...
class FullPageComponentLoader {
  ...
  async getCredentials(poolId, region, logins) {
    if (logins) {
      const client = new fullpage_component_loader_CognitoIdentityClient({
        region
      });
      const getIdentityId = new fullpage_component_loader_GetIdCommand({
        IdentityPoolId: poolId,
        Logins: logins
      });
      let identityId, getCreds;
      try {
        await client.send(getIdentityId).then(res => {
          identityId = res.IdentityId;
          getCreds = new fullpage_component_loader_GetCredentialsForIdentityCommand({
            IdentityId: identityId,
            Logins: logins
          });
        });
