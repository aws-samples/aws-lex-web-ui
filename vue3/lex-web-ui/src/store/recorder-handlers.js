...
const initRecorderHandlers = (context, recorder) => {
  ...
  recorder.ondataavailable = (e) => {
    ...
    context
      .dispatch('lexPostContent', audioBlob, offset)
      .then((lexAudioBlob) => {
        if (
          ...
          if (tmsg && Array.isArray(tmsg.messages)) {
            tmsg.messages.forEach((mes) => {
              context.dispatch('pushMessage', {
                ...
                alts: JSON.parse(context.state.lex.sessionAttributes.appContext || '{}')
                  .altMessages,
                ...
                responseCardsLexV2:
                  context.state.lex.sessionState &&
                  context.state.lex.sessionState.intent &&
                  (context.state.lex.sessionState.intent.state === 'Failed' ||
                    context.state.lex.sessionState.intent.state === 'Fulfilled')
                    ? context.state.lex.responseCardLexV2
                    : null
              })
            })
          }
      ...
      .then(() => {
        if (
          ['Fulfilled', 'ReadyForFulfillment', 'Failed'].indexOf(context.state.lex.dialogState) >= 0
        ) {
